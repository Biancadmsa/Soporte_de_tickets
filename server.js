...
app.get('/ticket/:id', autenticarToken, async (req, res) => {
  ...
  try {
    const resultado = await pool.query("SELECT tickets.*, usuarios.nombre AS nombre_usuario, tipos.nombre AS tipo FROM tickets JOIN usuarios ON tickets.id_usuario = usuarios.id JOIN tipos ON tickets.id_tipo = tipos.id WHERE tickets.id = $1 AND tickets.id_usuario = $2", [ticketId, req.usuario.id]);
    const ticket = resultado.rows[0];
    if (!ticket) {
      return res.status(404).send("Ticket no encontrado");
    }
    res.render("ticket_id", { cssFile: "ticket_id.css", title: "Detalle del Ticket", ticket, usuario: req.usuario });
  } catch (err) {
...
app.get('/ticket/aleatorio', autenticarToken, async (req, res) => {
  try {
    const resultado = await pool.query('SELECT * FROM tickets WHERE id_usuario = $1 ORDER BY RANDOM() LIMIT 1', [req.usuario.id]);
    const ticket = resultado.rows[0];
    if (!ticket) {
